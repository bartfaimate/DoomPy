...
class RayCasting:
    ...
    def get_objects_to_render(self):
        ...
        for ray, values in enumerate(self.ray_casting_result):
            ...
            if proj_height < config.HEIGHT:
                wall_column = self.textures[texture].subsurface(
                    offset * (config.TEXTURE_SIZE - config.SCALE), 0, config.SCALE, config.TEXTURE_SIZE
                )
                wall_column = pg.transform.scale(wall_column, (config.SCALE, proj_height))
                wall_pos = (ray * config.SCALE, config.HALF_HEIGHT - proj_height // 2)
            else:
    ...
    def ray_cast(self):
        ...
        ray_angle = self.game.player.angle - config.HALF_FOV + 0.0001
        for ray in range(config.NUM_RAYS):
            ...
            # projection
            proj_height = config.SCREEN_DIST / (depth + 0.0001)
