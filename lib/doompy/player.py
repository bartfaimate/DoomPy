...
class Player:
    ...
    def movement(self):
        ...
        dx, dy = 0, 0
        speed = config.PLAYER_SPEED * self.game.delta_time
    ...
    def check_wall_collision(self, dx, dy):
        scale = config.PLAYER_SIZE_SCALE / self.game.delta_time
    ...
    def draw(self):
        pg.draw.line(self.game.screen, 'yellow', (self.x * 100, self.y * 100),
                    (self.x * 100 + config.WIDTH * math.cos(self.angle),
                     self.y * 100 + config.WIDTH * math. sin(self.angle)), 2)
        pg.draw.circle(self.game.screen, 'green', (self.x * 100, self.y * 100), 15)

    def mouse_control(self):
        mx, my = pg.mouse.get_pos()
        if mx < config.MOUSE_BORDER_LEFT or mx > config.MOUSE_BORDER_RIGHT:
            pg.mouse.set_pos([config.HALF_WIDTH, config.HALF_HEIGHT])
        self.rel = pg.mouse.get_rel()[0]
        self.rel = max(-config.MOUSE_MAX_REL, min(config.MOUSE_MAX_REL, self.rel))
        self.angle += self.rel * config.MOUSE_SENSITIVITY * self.game.delta_time
